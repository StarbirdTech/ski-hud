...
def getObjects(vid, debug=False):
    ...
    if ret == True:
        ...
        leftScreenBounds = [[0, screenW], [
            frameH/2 - screenH/2, frameH/2 + screenH/2]]
        rightScreenBounds = [[frameW-screenW, frameW],
                             [frameH/2 - screenH/2, frameH/2 + screenH/2]]

        # left objects is all objects that are within left screen bounds
        leftObjects = [obj for obj in objects if obj[0] >= leftScreenBounds[0][0] and obj[0] <=
                       leftScreenBounds[0][1] and obj[1] >= leftScreenBounds[1][0] and obj[1] <= leftScreenBounds[1][1]]
        # right objects is all objects that are within right screen bounds
        rightObjects = [obj for obj in objects if obj[0] >= rightScreenBounds[0][0] and obj[0] <=
                        rightScreenBounds[0][1] and obj[1] >= rightScreenBounds[1][0] and obj[1] <= rightScreenBounds[1][1]]
        ...
        if debug:
            ...
            for i in range(0, len(leftObjects)):
                ...
                cv2.putText(frame, str(
                    i), leftObjects[i], cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, cv2.LINE_AA)
            # right screen objects
            for i in range(0, len(rightObjects)):
                cv2.circle(frame, rightObjects[i], 5, (255, 0, 0), -1)
                cv2.putText(frame, str(
                    i), rightObjects[i], cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
            ...
            for i in range(0, len(objects)):
                if objects[i] not in leftObjects and objects[i] not in rightObjects:
                    ...
                    cv2.putText(frame, str(
                        i), objects[i], cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
